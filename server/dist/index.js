...
io.on("connection", (socket) => {
    ...
    socket.on("join-room", ({ roomId, userName }) => {
        ...
        const users = Array.from(roomUsers.get(roomId).entries()).map(([socketId, name]) => ({
            socketId,
            userName: name,
        }));
    ...
    socket.on("canvas-state", (data) => {
        roomCanvasState.set(data.roomId, data.imageData);
    ...
    socket.on("disconnect", () => {
        ...
        if (roomId && roomUsers.has(roomId)) {
            ...
            if (usersInRoom.size === 0) {
                ...
                const users = Array.from(usersInRoom.entries()).map(([socketId, name]) => ({
                    socketId,
                    userName: name,
                }));
                io.to(roomId).emit("room-users", users);
